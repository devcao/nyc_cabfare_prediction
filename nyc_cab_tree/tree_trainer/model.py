...
## boosted tree only accept bucketized column and indicators
...
DEFAULTS = [[0.0], ['Sun'], [0], [-74.0], [40.0], [-74.0], [40.7], [1.0], [1.0]]
...
def build_estimator(model_dir, nbuckets):
    ...
    loc_time = tf.feature_column.crossed_column([pd_pair, day_hr], (nbuckets ** 4) * 24 * 7)

    b_pcount = tf.feature_column.bucketized_column(pcount, np.linspace(1, 10, 10).tolist())

    dayofweek_num = tf.feature_column.indicator_column(dayofweek)
    hourofday_num = tf.feature_column.indicator_column(hourofday)
    ...
    wide_columns = [
        ...
        #tf.feature_column.embedding_column(loc_time, 10),

        # Numeric columns
        b_plat, b_plon, b_dlat, b_dlon
        #latdiff, londiff, euclidean #loc_time
    ]
